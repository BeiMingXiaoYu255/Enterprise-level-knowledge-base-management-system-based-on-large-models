...
// 修改 base.html 中的相关脚本
// 移除原有的输入框提交按钮事件绑定，替换为：

// 绑定预设问题点击事件
document.querySelectorAll('.preset-question').forEach(btn => {
  btn.addEventListener('click', () => {
    this.selectedQuestion = btn.getAttribute('data-question');
    this.sendSelectedTextQueryWithPresetQuestion(this.selectedQuestion);
  });
});
// 绑定输入框提交按钮事件
