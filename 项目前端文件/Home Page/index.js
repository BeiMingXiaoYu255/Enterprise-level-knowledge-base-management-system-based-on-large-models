...
function initTechLinesAnimation() {
    ...
    // 定义蓝色调色板 - 使用与login页面一致的蓝色
    const colorPalette = {
        lightBlue: 0x5ac8fa,    // hsLightBlue
        mediumBlue: 0x00488b,   // hsBlue
        darkBlue: 0x00264d,     // hsDarkBlue
        white: 0xffffff
    };
...
function createTechElements() {
    const container = document.getElementById('indexTechBackground');
    if (!container) return;
    ...
    function updateCarousel() {
        ...
        indicators.forEach((indicator, index) => {
            if (index === currentSlide) {
                indicator.classList.add('bg-hsBlue');
                indicator.classList.remove('bg-gray-300');
            } else {
                indicator.classList.remove('bg-hsBlue');
    ...
    function handleScrollEffects() {
        ...
        if (mainNav) {
            if (window.scrollY > 100) {
                mainNav.classList.add('shadow-md', 'bg-white/95', 'backdrop-blur-md');
                mainNav.classList.remove('shadow-sm', 'bg-white/90');
            } else {
                mainNav.classList.remove('shadow-md', 'bg-white/95', 'backdrop-blur-md');
                mainNav.classList.add('shadow-sm', 'bg-white/90');
            }
        }
    }
